/**
 * Source from https://rawgit.com/rufuspollock/s3-bucket-listing/gh-pages/list.js
 */
...
function getS3Data(marker, html) {
  ...
  $.get(s3_rest_url)
    ...
    .fail(function (error) {
      console.error(error);
      $('#listing').html('<strong>Error</strong><pre>' + JSON.stringify(error, null, 2) + '</pre>');
    });
}
...
function prepareTable(info) {
  ...
  jQuery.each(files, function (idx, fileItem) {
    // strip off the prefix
    fileItem.keyText = fileItem.Key.substring(prefix.length);
    if (fileItem.Type === 'directory') {
      if (S3BL_IGNORE_PATH) {
        fileItem.href = location.protocol + '//' + location.hostname +
          location.pathname + '?prefix=' + fileItem.Key;
      } else {
        fileItem.href = fileItem.keyText;
      }
    } else {
      fileItem.href = BUCKET_WEBSITE_URL + '/' + encodeURIComponent(fileItem.Key);
      fileItem.href = fileItem.href.replace(/%2F/g, '/');
    }
    row = renderRow(fileItem, cols);
    content.push(row + '\n');
  });
